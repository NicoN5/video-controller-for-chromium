# Video Controller for Chromium

## About
This project is Chrome Extension for local use.
This extension lets you control videos on the current page.

## How to Install
1. Move to any directory you like.
2. Run this command: `git clone https://github.com/NicoN5/video-controller-for-chromium.git`.
3. Run this command: `npm install`.
4. Run this command to build the project locally: `npm run build`.
5. Open your browser.
6. Open the extension settings.
7. Enable "Developer Mode".
8. Click the "Load unpacked" button.
9. Select the "dist" directory generated by the previous steps.

## How to Use
![how-to-use-1](./images/readme-1.png)
1. Click the extension icon.
2. Videos on the current page are shown in the extension popup.
3. In the popup, click any video thumbnail you like.
4. Each button has a different function. For example, the "Time" slider controls playback time.
